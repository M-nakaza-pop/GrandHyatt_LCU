...
unsigned int rswScan(unsigned char num, unsigned int cnt){
	...
	if((lcnt[n]= ++lcnt[n] % cnt) ==0){
		...
		if((nowsw==ON)&&(swreg[n].b.b1==ON)&&(swreg[n].b.b2==ON)&&(swreg[n].b.b3==ON)){	// 新旧データ3回連続
			
			if(swreg[n].b.b4== OFF){	// 4スキャン前が"OFF"
			/* ONエッジ検出 */ 
...
char	swSearch(unsigned char num){
	...
	switch(num){			
		...
		case BSIDE:
			...
			break;
		case WORKS:
			if(ryCont(CARD,'r',0)==0){
				ryCont(DESK,'w',1);		// CARD有りではwelcome ONしない(誤動作防止の為)
				ryCont(FOOT,'w',1);
			}
			break;
		case CHIME:	
